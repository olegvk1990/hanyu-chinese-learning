module.exports = [
  {
    title: { zh: '结果补语 好/完/干净', ru: 'Результативные комплементы 好/完/干净', en: 'Result complements 好/完/干净' },
    explanation: { ru: 'Результативные комплементы ставятся после глагола и показывают результат действия: 好 (завершено хорошо), 完 (завершено полностью), 干净 (сделано чисто).', en: 'Result complements are placed after the verb to show the outcome: 好 (done well/ready), 完 (done completely), 干净 (done clean).' },
    pattern: 'verb + 好/完/干净 + 了',
    examples: [
      { chinese: '饭做好了，快来吃。', pinyin: 'Fàn zuòhǎo le, kuài lái chī.', translation: { ru: 'Еда готова, идите есть.', en: 'The meal is ready, come and eat.' } },
      { chinese: '作业写完了吗？', pinyin: 'Zuòyè xiěwán le ma?', translation: { ru: 'Ты дописал домашнее задание?', en: 'Have you finished the homework?' } },
      { chinese: '请把碗洗干净。', pinyin: 'Qǐng bǎ wǎn xǐ gānjìng.', translation: { ru: 'Вымой тарелки начисто.', en: 'Please wash the bowls clean.' } },
    ],
    difficulty: 'easy'
  },
  {
    title: { zh: '趋向补语 上/下/起来', ru: 'Направленные комплементы 上/下/起来', en: 'Directional complements 上/下/起来' },
    explanation: { ru: 'Направленные комплементы указывают направление действия: 上 (вверх, на), 下 (вниз, снять), 起来 (поднять, начать). Также имеют переносные значения.', en: 'Directional complements indicate direction: 上 (up, onto), 下 (down, off), 起来 (up, start). They also carry figurative meanings.' },
    pattern: 'verb + 上/下/起来',
    examples: [
      { chinese: '请把窗户关上。', pinyin: 'Qǐng bǎ chuānghù guānshàng.', translation: { ru: 'Пожалуйста, закрой окно.', en: 'Please close the window.' } },
      { chinese: '他脱下了外套。', pinyin: 'Tā tuōxià le wàitào.', translation: { ru: 'Он снял пальто.', en: 'He took off his coat.' } },
      { chinese: '大家笑了起来。', pinyin: 'Dàjiā xiào le qǐlái.', translation: { ru: 'Все рассмеялись.', en: 'Everyone burst out laughing.' } },
    ],
    difficulty: 'medium'
  },
  {
    title: { zh: '可能补语 得/不', ru: 'Комплемент возможности 得/不', en: 'Potential complement 得/不' },
    explanation: { ru: 'Между глаголом и результативным комплементом вставляется 得 (можно) или 不 (нельзя) для выражения возможности или невозможности действия.', en: 'Insert 得 (can) or 不 (cannot) between the verb and result complement to express possibility or impossibility.' },
    pattern: 'verb + 得/不 + result complement',
    examples: [
      { chinese: '这个箱子太重了，我搬不动。', pinyin: 'Zhège xiāngzi tài zhòng le, wǒ bān bù dòng.', translation: { ru: 'Этот ящик слишком тяжёлый, я не могу его сдвинуть.', en: 'This box is too heavy; I cannot move it.' } },
      { chinese: '你看得见黑板上的字吗？', pinyin: 'Nǐ kàn de jiàn hēibǎn shàng de zì ma?', translation: { ru: 'Ты видишь буквы на доске?', en: 'Can you see the characters on the blackboard?' } },
      { chinese: '这件衣服洗不干净了。', pinyin: 'Zhè jiàn yīfu xǐ bù gānjìng le.', translation: { ru: 'Эту одежду уже не отстирать.', en: 'This garment cannot be washed clean anymore.' } },
    ],
    difficulty: 'hard'
  },
  {
    title: { zh: '下来/下去 — 持续与完成', ru: '下来 и 下去 — завершение и продолжение', en: '下来 and 下去 — completion and continuation' },
    explanation: { ru: '下来 указывает на завершение или фиксацию результата (остановиться, записать). 下去 — на продолжение действия в будущем. Оба — направленные комплементы с переносным значением.', en: '下来 indicates completion or fixing a result (stop, write down). 下去 indicates continuation into the future. Both are directional complements with figurative meanings.' },
    pattern: 'verb + 下来 (fix result) | verb + 下去 (continue)',
    examples: [
      { chinese: '请把这个电话号码记下来。', pinyin: 'Qǐng bǎ zhège diànhuà hàomǎ jì xiàlái.', translation: { ru: 'Запиши этот номер телефона.', en: 'Please write down this phone number.' } },
      { chinese: '我们必须坚持下去。', pinyin: 'Wǒmen bìxū jiānchí xiàqù.', translation: { ru: 'Мы должны продолжать держаться.', en: 'We must keep persevering.' } },
      { chinese: '车慢慢停了下来。', pinyin: 'Chē mànman tíng le xiàlái.', translation: { ru: 'Машина медленно остановилась.', en: 'The car slowly came to a stop.' } },
    ],
    difficulty: 'hard'
  },
];
